...
import { homeCycleCards } from './data/thermodynamicLibrary';
...
const DualPage = React.lazy(() => import('./components/DualPage'));
const CombinedCyclePage = React.lazy(() => import('./components/CombinedCyclePage'));
...
const simulatorIcons = {
  rankine: Flame,
  brayton: Wind,
  otto: Zap,
  diesel: Flame,
  dual: Flame,
  'vapor-compression': Snowflake,
  carnot: RotateCw,
  combined: Factory,
};

const extraSimulators = [
  {
    title: 'Laboratorio Vapore',
    id: 'laboratorio-vapore',
    route: '/laboratorio-vapore',
...
const simulators = [
  ...homeCycleCards.map((cycle) => ({
    ...cycle,
    Icon: simulatorIcons[cycle.id] ?? Flame,
  })),
  ...extraSimulators,
];
...
const teachingHighlights = [
  'Preset didattici e confronto tra cicli vicini per ridurre gli errori piu comuni.',
  'Materiali leggibili anche da mobile durante studio individuale e ripasso veloce.',
  ...
  return (
    <Routes>
      <Route path="/" element={<Layout />}>
        ...
        <Route
          path="duale"
          element={<React.Suspense fallback={null}><DualPage /></React.Suspense>}
        />
        <Route
          path="frigo"
        ...
        />
        <Route
          path="ciclo-combinato"
          element={<React.Suspense fallback={null}><CombinedCyclePage /></React.Suspense>}
        />
        <Route
